...
const socket = io("https://server-dukungin-production.up.railway.app");   // ← GANTI kalau port atau domain berbeda

const overlayToken = "a07c7980f748b6d4b9e39bf1b56615b9";   // ← WAJIB GANTI INI!

socket.on("connect", () => {
    console.log("%c✅ Socket Connected!", "color: lime; font-size: 14px");
    console.log("Socket ID:", socket.id);
    
    socket.emit("join-room", overlayToken);
    console.log(`📡 Join room: ${overlayToken}`);
});
...
socket.on("new-donation", (data) => {
    console.group(`%c🎉 DONASI MASUK - Rp${data.amount}`, "color: gold; font-size: 16px; font-weight: bold");
    console.log("Donor:", data.donorName);
    console.log("Pesan:", data.message || "-");
    console.log("Full Data:", data);
    console.groupEnd();

    alert(`💰 Donasi dari ${data.donorName}\nRp ${data.amount}\n\n"${data.message || 'Tanpa pesan'}"`);
});

console.log("%c🚀 Socket setup selesai. Sekarang coba kirim test donation.", "color: cyan");

// ==================== KIRIM TEST DONATION ====================

socket.emit("new-donation", {
    donorName: "Test Donatur",
    amount: 100000,
    message: "Ini testing dari console browser! 🔥 Mantap bro",
    mediaUrl: null,
    mediaType: "image"
});

console.log("%c📤 Test donation sudah dikirim!", "color: cyan");